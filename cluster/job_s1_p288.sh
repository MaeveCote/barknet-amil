#!/bin/bash
# STAGE 1 ONLY -- patch 288, all 5 folds. Pretrains the backbone; Stage 2 is a separate
# array (job_s2_p288.sh) chained behind this via aftercorr in submit_288.sh.
# Walltime = measured 40-epoch Stage-1 time + margin, kept short for gpubackfill.
#SBATCH --account=def-oberman_gpu
#SBATCH --job-name=bark_s1_p288
#SBATCH --array=0-4
#SBATCH --gres=gpu:1
...
export STAGES=1
export EPOCHS_S1=40
export RUN_NAME="abl_p288_nano_f${FOLD}"
