#!/bin/bash
# MODEL-SIZE ABLATION -- Stage 1, pico @ patch 224, 5 folds. Uses the SAME 224 patch tar as
# the patch-size study, so field of view is fixed and BACKBONE CAPACITY is the only variable.
# Epochs: 40 (pico converges within this; tiny trimmed to fit the 24h wall).
#SBATCH --account=def-oberman_gpu
#SBATCH --job-name=bark_s1_pico224
#SBATCH --array=0-4
#SBATCH --gres=gpu:1
